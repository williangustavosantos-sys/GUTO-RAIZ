...
- [x] **✅ CABEADO 02/06 / ⏸️ ~~ligar a chave~~ ADIADO (opcional, pago).** O `release:gate` já **liga o juiz LLM automaticamente se `ANTHROPIC_API_KEY` existir**; sem a chave, degrada para match por palavra-chave com aviso. **Decisão do fundador: NÃO ativar agora** — a Anthropic é paga (sem free tier) e o chat já foi verificado ao vivo (8/8). Fica pronto pra ligar quando/se quiser. (CEREBROGUTO) [03]
